...
1. **Workspace state** — `workspaces.json` in `%APPDATA%\ymux`. Names, hosts, ports, paths. Not a secret per se but reveals where the user works.
...
3. **Agent tokens** — Claude Code session state lives under `~/.claude` and `~/.codex` on the *remote* boxes; ymux itself doesn't store provider API keys.
4. **Audit log** — `debug.log` in `%APPDATA%\ymux`. PTY content is NEVER written there (CLAUDE.md Rule 1); only event metadata.
...
- **A3 — Prompt injection in a document.** A README, a PDF, or output a tool produces contains text that instructs the agent to do something off-script. The agent itself is in scope of its own provider's safety; what ymux can prevent is the agent escalating into actions the user didn't approve.
...
Explicitly **out of scope**: local Administrator, SYSTEM, kernel-mode adversaries, physical access to an unlocked machine, side-channel attacks. If the attacker is admin on the local box, ymux is not the layer that helps.

## Mitigations by adversary

### A1 (same-user processes)

- Named pipe ACL restricts to the current Windows user (default Tauri behavior + the pipe is created without explicit broader DACL). A second non-user process trying to connect gets ACCESS_DENIED before any ymux code runs.
- We DO authenticate over the pipe too (HMAC challenge/response from the reverse tunnel; the bundled CLI dials in with a token only ymux itself knows for that session). This isn't defense against A1 specifically — it's defense against a remote process that's been given a pipe handle by accident.

### A2 (malicious postinstall on the remote)

- Reverse-tunnel auth: only callers that know `YMUX_TUNNEL_TOKEN` (a per-session 32-char random) can complete the RPC handshake. The token lives only in the SSH session's env, in the env file `~/.ymux/run/last.env` (mode 0600), and in our process memory.
- Bridge handshake (`tunnel.rs::perform_handshake`) rejects clients with the wrong token before any other byte is read.
- Forward auto-fetching is OFF by default and only fires for ports the WATCHER reported as LISTENING (Phase 39+). ymux's own reverse-tunnel port is explicitly filtered (Phase 39 C2 — also defended in the CLI via `YMUX_SOCKET_ADDR` self-exclude).
- Forwards are LOCAL-only (`127.0.0.1`, not `0.0.0.0`); a forwarded port is not exposed to the LAN.

### A3 (prompt injection)

- Hooks system intercepts Claude Code permission requests on the remote and routes them through ymux (the OS-level toast / Feed item asks the *user*, not the model, for the decision). Default policy preset prompts on risky tools; the relaxed and auto presets are opt-in.
- The agent runs in its own process tree on the remote; it cannot inject into ymux's local UI by emitting text — at most it can persuade the user via what shows up in their terminal pane, which is the same risk as reading any text from a remote shell.

### A4 (compromised remote)

- Host key check (russh) trips on first connect and on key change. The `UNKNOWN_HOST` and `HOST_KEY_MISMATCH` errors surface in the UI and require explicit user confirmation. Known hosts stored at `%APPDATA%\ymux\known_hosts`.
...
- We do not sandbox tool execution on the remote — the agent runs as the user, full PATH, full FS access. Sandboxing would have to be at the remote OS / container layer; ymux is the wrong place.
...
- The bundled MCP exe (`ymux-mcp.exe`) connects to the same named pipe with the same auth surface as the user CLI. A future hardening: make MCP authenticate with a separate per-process token rather than the shared one.
