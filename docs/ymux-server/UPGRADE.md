# Upgrading to ymux-server 2.0 (from ymux-insights 1.x)

Phase 77 renamed the daemon `ymux-insights` → **`ymux-server` 2.0.0** and
...
1. Uploads the new binary to `~/.ymux/bin/ymux-server` and points the
   `ymux-insights` symlink at it (so version probes keep working).
2. Disables + removes the old `ymux-insights` systemd unit; installs and starts
   `ymux-server` (see [DEPLOYMENT.md](DEPLOYMENT.md)).
3. On its first boot the 2.0 daemon **migrates the data dir
   `~/.ymux/insights/` → `~/.ymux/server/`** — an atomic whole-directory
   move (both under `~/.ymux`, same filesystem). `token`, `metrics.db`,
...
The 2.0 daemon moved the data dir to `~/.ymux/server/`. To roll back to a 1.x
release (which reads `~/.ymux/insights/`), move the data back first:

```sh
mv ~/.ymux/server ~/.ymux/insights   # then install the older ymux release
```

A 1.x binary started without this move would create a fresh, empty
`~/.ymux/insights/` (losing the paired devices + token), so do the move before
