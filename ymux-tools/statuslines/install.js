#!/usr/bin/env node
// ymux-tools statuslines installer (local prototype of the Half B statusLine
// deploy). Copies this statuslines/ folder into ~/.claude/ymux-tools/ and
...
//
// MERGE, never clobber: existing hooks (sabra-prompt.js, ymux-cli, etc.) are
// preserved — we only ADD our own entries and only replace a prior ymux-tools
...
const SRC = __dirname; // ymux-tools/statuslines
const DEST = path.join(CLAUDE, 'ymux-tools', 'statuslines');
const isOurs = (s) => typeof s === 'string' && s.includes('ymux-tools');
const q = (p) => `node "${p}"`;
...
function main() {
  ...
  console.log('  hooks  : UserPromptSubmit/Stop/SessionEnd += ymux-tools turn-state');
