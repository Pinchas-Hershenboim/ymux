...
A modular home for Claude Code add-ons that ymux can install per-workspace.
Split in two:
...
| `turn-ticker` | **session Ticker** — current turn elapsed + rolling avg (`YMUX_TICKER`) |
...
State lives at `~/.claude/ymux-tools/turn-state/<session_id>.json`. Turns
under 2s (text-only) are excluded from the average.

The same turn signals also feed a ymux **chrome ticker** (a per-pane label in
the app), via ymux's own `UserPromptSubmit`/`Stop` hooks — see the backend.

## `skills/`

A registry the user drops their own skills into; ymux installs them
per-workspace onto each environment's `~/.claude/skills/` (local, or remote
over SFTP), modeled on ymux's existing Add-on manager.
