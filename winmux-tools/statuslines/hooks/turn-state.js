#!/usr/bin/env node
// winmux-tools turn-state hook — powers the session Ticker (issue #4).
//
...
// ~/.claude/winmux-tools/turn-state/<sid>.json = { turn_started_at, sum_ms, count }.
// Never blocks the prompt/turn (always exit 0). Reads Claude's hook JSON on stdin.
const fs = require('fs');
const os = require('os');
const path = require('path');

const DIR = path.join(os.homedir(), '.claude', 'winmux-tools', 'turn-state');
