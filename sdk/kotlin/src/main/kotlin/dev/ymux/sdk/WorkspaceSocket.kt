...
// (8a/8b). Deserializes server frames into the generated YmuxFrame sealed
...
class WorkspaceSocket private constructor(private val ws: WebSocket) {
    ...
    private fun send(obj: JsonObject) { ws.send(YmuxJson.instance.encodeToString(JsonObject.serializer(), obj)) }
        ...
        fun subscribe(
            ...
            val socket = http.newWebSocket(req, object : WebSocketListener() {
                override fun onMessage(webSocket: WebSocket, text: String) {
                    try {
                        handler.onFrame(YmuxJson.instance.decodeFromString(YmuxFrame.serializer(), text))
                    } catch (t: Throwable) {
                        handler.onError(t)
                    }
                }
