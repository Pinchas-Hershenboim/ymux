// YmuxClient.kt — hand-written OkHttp client for ymux-server's REST surface,
...
class YmuxApiException(val status: Int, val bodyText: String) :
    RuntimeException("ymux-server $status: $bodyText")
...
class YmuxClient(
    ...
    internal fun getText(path: String): String =
        http.newCall(req(path).get().build()).execute().use { res ->
            ...
            if (!res.isSuccessful) throw YmuxApiException(res.code, body)
            body
        }
    ...
    fun readFile(path: String, maxBytes: Long? = null): FileBytes {
        ...
        http.newCall(req("/api/v2/files/read?path=${enc(path)}$q").get().build()).execute().use { res ->
            if (!res.isSuccessful) throw YmuxApiException(res.code, res.body?.string().orEmpty())
            return FileBytes(res.body!!.bytes(), res.header("X-Ymux-Truncated") == "true")
        }
    }
    ...
    fun uploadFile(path: String, data: ByteArray, filename: String = "file"): UploadResultBody {
        ...
        http.newCall(req("/api/v2/files/upload?path=${enc(path)}").post(body).build()).execute().use { res ->
            ...
            if (!res.isSuccessful) throw YmuxApiException(res.code, txt)
