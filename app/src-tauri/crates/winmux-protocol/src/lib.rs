//! Phase 67.A.1 — winmux mobile/web wire protocol.
//!
//! Shared types between the winmux **server** (`winmux serve`, runs on the
...
//! consumer links it cheaply, mirroring `winmux-types` / `winmux-policy`.
//!
...
/// Default TCP port for `winmux serve` (overridable in settings).
...
wire_type! {
    ...
    // No `Debug`: ServerMsg::PaneList embeds winmux_types::LayoutNode,
    ...
    pub enum ServerMsg {
        ...
        PaneList {
            workspace_id: String,
            layout: Option<winmux_types::LayoutNode>,
        },
...
wire_type! {
    ...
    /// `winmux_types::Workspace` — the client only needs these to render
