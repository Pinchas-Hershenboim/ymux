        ...
        tokio::spawn(async move {
            ...
            if let Some(h) = handle {
                if let Err(e) = crate::updater::ssh_exec_simple(&h, &origin_cmd).await {
                    dlog(&format!("session-meta: origin write failed: {e}"));
                }
            }
        });
    }
    ...
    if let Some((_sid, handle, tmux_name)) = tmux_target {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = crate::updater::ssh_exec_simple(&handle, &cmd).await {
                dlog(&format!("session-meta: label write failed: {e}"));
            }
        });
    }
...
async fn pane_list_tmux_sessions(
    ...
    // Phase 81: split the combined output — sessions above the marker,
    // the session-meta JSON below it. Garbled/absent JSON degrades to no
